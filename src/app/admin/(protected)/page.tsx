...
type RecentOrdersProps = {
  recentOrdersPromise: ReturnType<typeof listAdminOrders>;
};

async function SummaryStats({ statsPromise }: SummaryStatsProps) {
...
async function RecentOrders({ recentOrdersPromise }: RecentOrdersProps) {
...
export default function AdminResumenPage() {
  ...
  const recentOrdersPromise = listAdminOrders({ page: 1, limit: 5 });
  ...
  return (
    <div className="space-y-8">
      ...
      <Suspense fallback={<AdminSummaryStatsLoading />}>
        <SummaryStats statsPromise={statsPromise} />
      </Suspense>

      <Suspense fallback={<AdminRecentOrdersLoading />}>
